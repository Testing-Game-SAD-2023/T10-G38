  ...
  const handleConfirm = () => {
    ...
    fetch("http://localhost:8080/game/add", {
      ...
      .then(data => {
        ...
        let editorapi = "http://localhost:8190?myParam=" + encodeURIComponent(selectedClass) + "&id=" + encodeURIComponent(gameid) + "&robot="

        if (selectedRobot == "Randoop") {
          editorapi += encodeURI("R")
        } else {
          editorapi += encodeURI("E")
        }

        window.location.href = (editorapi)
      })
      .catch(error => {
